...
def pro_predskaz(niz,vverh, konec,prd_posl_cifra, posledn_cifra):  #niz от куда начать считать статистику (vverh =niz+1) konec где закончить
    cikl_v_rabote = True                                           #prd_posl_cifra на строке 743 переменная
    niz_granicu = niz                                              #posledn_cifra  на строке 743 переменная
    vrh_granicu = vverh
    konecc = konec
    old_summa_of = 0
    list_of = [0, 0, 0]
    ...
    while cikl_v_rabote:
        ...
        list_of = vudacha(prd_posl_cifra, posledn_cifra, sorted_dvoyki)
        ...
        if (old_summa_of - summa_of) != 0:
            print('znachenija: ', list_of[0], ' ', list_of[1], ' ', list_of[2], ' ', for_print[0], rez_odin_str, '',
                  for_print[1], rez_dva_str, '', for_print[2], rez_tri_str)
