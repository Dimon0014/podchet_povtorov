...
for i in range(222, 1634):  # while (ik < 1): # количество файлов
    ...
    while (steps < len(viborka)):
        ...
        steps = steps + 1
        if old_key == 0:
            list_0.append((old_key,key1))
        if key1 == 0:
            podchet_0 = podchet_0+1
        if old_key > -1:
            # list_dvoek.append( str(key1) + '  ' + str(old_key))
            list_dvoek.append( (old_key,key1))
        if old_key == 0 and key1 == 23:
            podchet_0_23 = podchet_0_23+1
            pribul_0 = pribul_0 +36
            list_promejutkov.append(podchet_0)
            print('glob_hsag:', global_shagi, 'sootnoshenie:', podchet_0, podchet_0_23, '  ', naime_file)
            podchet_0 = 0
            podchet_0_23 =0
...
after0 = get_counts(list_0)
# print('dvoyki: ', dvoyki)
import operator

sorted_dvoyki = sorted(dvoyki.items(), key=operator.itemgetter(1), reverse=True)
sorted_0 = sorted(after0.items(), key=operator.itemgetter(1), reverse=True)
print('0: ', podchet_0)
print('sorted_dvoyki: ', sorted_dvoyki)
print('sorted 0: ', sorted_0)
