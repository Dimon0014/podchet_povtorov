...
for i in range(1600, 1634):  # while (ik < 1): # количество файлов
    ...
    while (steps < len(viborka)):
        ...
        if old_key == 35:
            list_35.append((old_key,key1))
        if key1 == 35:
            podchet_35 = podchet_35 + 1
        if old_key > -1:
            # list_dvoek.append( str(key1) + '  ' + str(old_key))
            list_dvoek.append( (old_key,key1))
        if old_key == 35 and key1 == 16:
            podchet_35_16 = podchet_35_16 + 1
            pribul_35 = pribul_35 +36
            list_promejutkov.append(podchet_35)
            print('glob_hsag:', global_shagi, 'sootnoshenie:', podchet_35,podchet_35_16, '  ', naime_file)
...
after35 = get_counts(list_35)
# print('dvoyki: ', dvoyki)
import operator

sorted_dvoyki = sorted(dvoyki.items(), key=operator.itemgetter(1), reverse=True)
sorted_35 = sorted(after35.items(), key=operator.itemgetter(1), reverse=True)
print('35: ', podchet_35)
print('sorted_dvoyki: ', sorted_dvoyki)
print('sorted 35: ', sorted_35)
